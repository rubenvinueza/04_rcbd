...
library(lme4) # to run mix effect Anova model 
...
- Residual independence (no pattern). We do not want to see patterns, comes strongly that we randomize those obeservations. 
- Residual variance homogeneity (homoscedasticity): the variance is consistent across data set, not lower variance in some places and large variance in other places. 
- Residual normality (discuss this!): they are normal distributed or not.  
